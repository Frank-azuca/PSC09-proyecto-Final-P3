...
namespace PSC09
{
    public partial class frmFactura : Form
    {
        ...
        {
            SqlConnection cxn = new SqlConnection(cnn.db); cxn.Open();
            SqlCommand cmd = new SqlCommand("SELECT ITEM, DESCRIPCION, PRECIOVENTA, CANTIDAD, IMPUESTO FROM PRODUCTOS WHERE ITEM = '" + nmrArticulo + "'", cxn);

            SqlDataReader rdr = cmd.ExecuteReader();

            if (rdr.Read())
            {
                Double existencia = Convert.ToDouble(rdr["Cantidad"].ToString());
                if(existencia > 0)
                {
        ...
        private void BorrarData(string numFactura)
        {
            if (ExisteLaData == true)
            {
                ...
                cms.ExecuteNonQuery();
                BorrarAuxiliar(lblFactura.Text);
            }
        }

        private void BorrarAuxiliar(string numFactura)
        {
            if(ExisteLaData == true)
            {
                SqlConnection cns = new SqlConnection(cnn.db); cns.Open();
                string ssQuery = "DELETE FROM MUTOCTE WHERE FACTURA = '" + numFactura + "'";
                SqlCommand cms = new SqlCommand(ssQuery, cns);
                cms.ExecuteNonQuery();
            }
        ...
        private void BuscarFactura(string nmrFactura)
        {
            ...
            SqlConnection cnx = new SqlConnection(cnn.db); cnx.Open();
            string tsQuery = " SELECT A.FACTURA, A.CLIENTE, B.NOMBRE, A.FECHA, A.SUBTOTAL, A.IMPUESTO, A.MONTOFACTURADO " +
                             " FROM HFACTURA A INNER JOIN CLIENTES B ON A.CLIENTE = B.IDCLIENTE " + 
                             " WHERE A.FACTURA = '" + nmrFactura + "' AND A.ACTIVO = '1' ";
        ...
        private void InsertarAuxiliar()
        {
               string stQuery = " INSERT INTO MUTOCTE (IDCLIENTE, FECHA, ORIGEN, DOCUMENTO, APLICADO, MONTO, BCPENDIENTE, ACTIVO) " +
                ...
                cmd.Parameters.AddWithValue("@A1", DateTime.Now.ToString("yyyy/MM/dd"));
                cmd.Parameters.AddWithValue("@A2", "1");
                cmd.Parameters.AddWithValue("@A3", lblFactura.Text);
                cmd.Parameters.AddWithValue("@A4", "00000000");
                cmd.Parameters.AddWithValue("@A5", lblTotal.Text);
        ...
        private void ActualizaInventario(string numProducto, string laCantidad)
        {
            SqlConnection cns = new SqlConnection(cnn.db); cns.Open();
            string ssQuery = "UPDATE PRODUCTOS SET CANTIDAD = (CANTIDAD - '" + laCantidad +"') WHERE ITEM = '" + numProducto + "'";
        ...
        private void InsertaDetalleFactura()
        {
            ...
            for (int xrow = 0; xrow < dgv.Rows.Count ; xrow++)
            {
                ...
                ActualizaInventario(nmArt, nmCan);
            }
        }
